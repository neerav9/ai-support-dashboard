  ...
  return (
    <div className={cn("flex gap-3", isUser ? "flex-row-reverse" : "")}>
      ...
      >
        ...
        <div
          className={cn(
            "px-4 py-2.5 shadow-lg shadow-black/30 ring-1 ring-inset transition-all duration-200",
            isUser
        ...
        >
          {isUser ? (
            ...
            <div className="prose prose-sm prose-invert max-w-none prose-p:my-2 prose-p:text-[14px] prose-p:leading-relaxed prose-p:text-zinc-100 prose-headings:text-zinc-50 prose-strong:text-zinc-50 prose-a:text-cyan-400 prose-a:no-underline hover:prose-a:underline prose-ul:my-2 prose-ol:my-2 prose-li:my-0.5 prose-li:text-[14px] prose-li:text-zinc-200">
              <ReactMarkdown
                components={{
                  code({ className, children }) {
                    ...
                    if (!match) {
                      return (
                        <code className="rounded-md border border-zinc-800 bg-zinc-950 px-1.5 py-0.5 font-mono text-[12.5px] text-cyan-400">
                          {children}
                        </code>
                      )
                    }
...
function ChatHeader({
  ...
  return (
    <div className="flex h-14 shrink-0 items-center justify-between border-b border-zinc-800/80 bg-zinc-950/60 px-4 backdrop-blur-xl">
      <div className="flex items-center gap-3">
        {onToggleSidebar && (
          <Button
            ...
            className="size-8 text-zinc-400 transition-colors hover:bg-zinc-900 hover:text-zinc-100"
          >
            {sidebarOpen ? (
        ...
        <span className="h-4 w-px bg-zinc-800" />
        <h2 className="text-[14px] font-semibold tracking-tight text-zinc-100">
          Debugging API issues
        </h2>
      </div>
      <div className="flex items-center gap-2 rounded-full border border-zinc-800 bg-zinc-900/60 px-2.5 py-1 backdrop-blur-sm">
